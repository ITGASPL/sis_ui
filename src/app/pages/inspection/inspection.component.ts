...
export class InspectionComponent implements OnInit, OnDestroy {
  ...
  downloadStyledExcel1(data: any[]) {
    ...
    const worksheet = workbook.addWorksheet('Sheetwise Summary Report');

    const titleRow = worksheet.addRow(['Sheetwise Summary Report']);
    ...
    data.forEach((item, index) => {
      worksheet.addRow([
        ...
        item.reportStartDate
          ...
          : '',
        item.reportEndDate ? new Date(item.reportEndDate).toLocaleString() : '',
        item.rejectionLogTime
          ? new Date(item.rejectionLogTime).toLocaleString()
          : '',
        item.rejectionQuantity || '',
        item.sheetNumber || '',
        item.inspectionResult || '',
        item.operatorDefectTypes || '',
      ]);
    });
  ...
  getCurrentRecipeInformationData() {
    this.currentRecipeInformationService
      .getCurrentRecipeInformationData()
      .subscribe({
        next: (response: any) => {
          this.currentRecipeData = response;

          this.leftOffset1 = response.vartiantMasterDTO.lo1
            ? response.vartiantMasterDTO.lo1
            : 0;
          ...
          this.depthOffset = response.vartiantMasterDTO.depthOffset
            ? response.vartiantMasterDTO.depthOffset
            : 0;
          this.variantType = response?.vartiantMasterDTO?.varientType;
          if (response?.vartiantMasterDTO?.varientType === 'Oscillator') {
            this.leftOffset2 = response.vartiantMasterDTO.lo2
              ? response.vartiantMasterDTO.lo2
              : 0;
  ...
  getAllNgSheetsForCurrentDyeData(): Observable<any> {
    return this.allNGSheetCurrentDyeService.getAllNGSheetCurrentDye().pipe(
      tap((res: any) => {
        this.summaryData = res.listOfSheetWiseSummuryDto;
      }),
      catchError((err) => {
